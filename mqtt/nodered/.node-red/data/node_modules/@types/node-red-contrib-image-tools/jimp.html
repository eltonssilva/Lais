...
<script type="text/javascript">
    ...
    imageToolsNodeCommon.functions = {
        ...
        "quality" : {"name" : "quality","fn" : "quality", "description" : "Set the quality of the image. Usefull for reducing size of image before calling the write function.","parameters" : [
        ...
        {"name" : "src", "type" : "", "required" : true, "hint" : "the source Jimp instance", "defaultType" : "msg", "defaultValue" : "payload" },
        ...
        {"name" : "val", "type" : "num", "required" : true, "hint" : "adjust the brighness by a value -1.0 to 1.0" }] },
        ...
        {"name" : "map", "type" : "", "required" : true, "hint" : "the source Jimp instance", "defaultValue" : "payload" },
...
//not ready        "histogram" : {"name" : "histogram","fn" : "histogram", "description" : "Generate a histogram of the image. The histogram data will be returned in msg.jobs[x].result, msg.jobs[x].g[], msg.jobs[x].b[].","parameters" : [] },
        ...
        {"name" : "font", "type" : "jimpFont", "required" : true, "hint" : "font to print" },
        ...
        {"name" : "maxHeight", "type" : "num", "required" : false, "hint" : "" }] },
        ...
        {"name" : "maxHeight", "type" : "auto|num", "required" : false, "hint" : "" }] }
    ...
    RED.nodes.registerType('jimp-image', {
        ...
        outputs: 1,
        label: function () {
            let fname = this.selectedJimpFunction && this.selectedJimpFunction.name ? this.selectedJimpFunction.name : ""; 
            if(fname == "none") fname = "";
            return this.name || fname || "image";
        },
        oneditsave: function() {
            let node = this;
            let $dropdown = $("#node-input-jimpFunction");
        ...
        oneditprepare: function () {
            ...
            if(!imageToolsNodeCommon.alignModes){
                ...
                for (let key in imageToolsNodeCommon.alignMode) {
                    if (imageToolsNodeCommon.alignMode.hasOwnProperty(key)) {
                        imageToolsNodeCommon.alignModes.push(key);
                    }
                }
            }
            function setupTypedInput(varName, allowableTypes, def, defVal, isOptional) {
                let varSel = "#node-input-" + varName;
                let typeSel = varSel + "Type";
                let currentValue = node[varName];
                let currentType = node[varName + "Type"];
                allowableTypes = allowableTypes || "msg|flow|global";
                let parameterTypes = allowableTypes.split("|");
                let isStringInArray = function(arr,str){
                ...
                if(typeof isOptional == "boolean" && isOptional == false){
                    if(!isStringInArray(parameterTypes,"none") ){
                ...
                if(!isStringInArray(parameterTypes, def)){
                    def = parameterTypes[0];
                }

                //if currently set type is not in the list of parameter types, currentType def to 1st item
                if(!isStringInArray(parameterTypes, currentType)){
                    currentType = parameterTypes[0];
                }
                
                //convert any custom types to a typedInput object
                let types = parameterTypes.map(function(t){
                    if(t){
                        switch (t) {
                            case "resizeMode":
                                return {
                ...
                else if(currentType == "bool" && (currentValue == "true" || currentValue == "false"))
                    $ti.typedInput("value", currentValue);
                else if(defVal)
                    $ti.typedInput("value", defVal);
            }
            
            setupTypedInput('data', 'msg|flow|global|bin|str|json', 'msg', 'payload', false);
            
            let $dropdown = $("#node-input-jimpFunction");
            $dropdown.off();
            $dropdown.empty().append('<option value=""></option>')
            for (let key in imageToolsNodeCommon.functions) {
                if (imageToolsNodeCommon.functions.hasOwnProperty(key)) {
                    const element = imageToolsNodeCommon.functions[key];
                    if(node.jimpFunction == key){
                        $dropdown.append($("<option selected />").val(key).text(element.name));
                    } else {
                        $dropdown.append($("<option />").val(key).text(element.name));
                    }
                }
            }
            //let index = 0;
            // Object.keys(imageToolsNodeCommon.functions).forEach(function(key,index) {
            //     // key: the name of the object key
            //     // index: the ordinal position of the key within the object 
            //     let element = imageToolsNodeCommon.functions[key];
            //     if(node.jimpFunction == key){
            //         $dropdown.append($("<option selected />").val(key).text(element.name));
            //     } else if(index == node.jimpFunction)
            //         $dropdown.append($("<option selected />").val(key).text(element.name));
            //     else    
            //         $dropdown.append($("<option />").val(key).text(element.name));
            // });

            let $convertToBatchButton = $("#convertToBatchButton");

            $convertToBatchButton.click(function() {
                try {
                    let choice = $dropdown.val(); 
                    let fn = imageToolsNodeCommon.functions[choice];  
                    let paramCount = fn && fn.parameters ? fn.parameters.length : 0;
                    let batchSpec = {
                        name: fn.fn,
                        parameters:[]
                    }
                    ...
                    for (let i = 0; i < paramCount; i++) {
                        ...
                        let $ti = $('#node-input-parameter' + (i + 1));
                        let pv = $ti.typedInput('value');
                        let pt = $ti.typedInput('type');
                        if(pt === 'msg' || pt === 'global' || pt === 'flow' || pt == "none"){
                            ...
                            let value = imageToolsNodeCommon.alignMode[pv] || 0;
                    ...
                    const isNotUndefined = function(v){
                        return (v !== 'undefined')
                    }
                    let finalParams = [];
                    let fplookup = {};
                    ...
                    for (let index = 0; index < fn.parameters.length; index++) {                       
                        ...
                        if(funcParam.group){
                            ...
                            if(isNotUndefined(value)){
                                finalParams[i][funcParam.name] = inputParameters[index];
                            }
                        } else {
                            let value = inputParameters[index];
                            if(isNotUndefined(value)){
                                finalParams[fParam++] = inputParameters[index];
                            }
                        }
                    }  
                    batchSpec.parameters = finalParams;
                    $dropdown.val("batch").change();
                    let p1NewValue = JSON.stringify(batchSpec);
            ...
            $dropdown.change(function() {
                ...
                $.each(fn.parameters, function() {
                    let pname = "parameter" + paramNo;
                    let lname = "parameter-label" + paramNo;
                    let paramTipName = "tip-parameter" + paramNo;
                    
                    $("#" + lname).text(" " + this.name);
                    $("#" + pname).show();

                    let hint = (this.required ? "(Required)" : "(Optional)");
                    if(this.hint){
                        hint += " " + this.hint;
                    }
                    ...
                    //hack to cause typedInput widget to be destroyed so it can be re-created later
                    //gotta be a more elegant way but i've seen issues with the widget being recrated with old one still present, input box still hidden etc etc
                    try {
                        let $theInput = $('#node-input-' + pname);
                        $theInput.attr("type","text");
                        let $typedInput = $('#' + pname + ' > div > div.typed-parameter > div');
                        $('#' + pname + ' > div.input-tip-group > div.typed-parameter > div > div > input[type="hidden"]').attr("type","text");
                        try {
                            $theInput.typedInput("destroy");
                        } catch (error) {
                            //console.error(error) 
                        }
                        $theInput.removeClass("red-ui-typedInput");
                        if($typedInput.hasClass("red-ui-typedInput-container") /*&& $typedInput.hasClass("paramtip") == false*/){    
                            $typedInput.remove();     
                        }
                    } catch (error) {
                        //console.error(error)
                    ...
                    const alwaysPresentParameterTypedInputs = "msg|flow|global";
                    const parameterTypesStr = this.type ? this.type + "|" + alwaysPresentParameterTypedInputs : alwaysPresentParameterTypedInputs;
                    setupTypedInput(pname, parameterTypesStr, this.defaultType, this.defaultValue, this.required);

                    try {
                        $('#node-input-parameter' + paramNo).typedInput('width', '98%');//hack to size it to div
                    } catch (error) {
...
<script type="text/html" data-template-name="jimp-image">
    ...
    <div class="form-row">
        <div style="float:left;">
            <label style="padding-top: 8px" for="node-input-data"><i class="fa fa-file-image-o"></i><span data-i18n="jimp.label.data">
                image</span></label>
        </div>
        <div class="input-tip-group">
            <div class="typed-parameter">
                <input type="hidden" id="node-input-dataType">
                <input type="text" id="node-input-data" placeholder="">
            </div>
            <div class="form-tips">A string containing a file path, URL or base64 image can be used as an image source. NOTE: Passing in an image object is faster as there is no conversion required before processing.</div>
        </div>
    </div>
    
    <div class="form-row" style="vertical-align: top;">
        <div style="float:left;">
        ...
        <div class="input-tip-group">
            <div>
                <span><select type="text" id="node-input-jimpFunction" style="width:92%;"></select></span>
                <span><button id="convertToBatchButton" class="ui-button button" title="convert to batch function"><i
                            class="fa fa-exchange"></i></button></span>
            </div>
            <div class="form-tips" id="tip-jimpFunction" hidden></div>
        </div>
    </div>
    ...
    <div class="form-row">
        <div style="float:left;">
            <label style="padding-top: 8px" for="node-input-ret"><i class="fa fa-arrow-left"></i> <span data-i18n="jimp.label.return"></span>
                return</label>
        </div>
        <div class="input-tip-group">
            <select type="text" id="node-input-ret" style="width:100%;">
            ...
            <div class="form-tips">Returning an image is much faster as there is no additional conversion required.</div>
        </div>    
    </div>
    
    
    <div class="form-row jimp-parameter-group" id="parameter1" style="vertical-align: top;">
        <div style="float:left;">
        ...
        <div class="input-tip-group">
            <div class="typed-parameter">
                ...
                <input type="text" id="node-input-parameter1" placeholder="">
            </div>
            <div class="form-tips paramtip" id="tip-parameter1" hidden></div>
        </div>
    </div>
    <div class="form-row jimp-parameter-group" id="parameter2" style="vertical-align: top;">
        <div style="float:left;">
        ...
        <div class="input-tip-group">
            <div class="typed-parameter">
                ...
                <input type="text" id="node-input-parameter2" placeholder="">
            </div>
            <div class="form-tips paramtip" id="tip-parameter2" hidden></div>
        </div>
    </div>
    <div class="form-row jimp-parameter-group" id="parameter3" style="vertical-align: top;">
        <div style="float:left;">
        ...
        <div class="input-tip-group">
            <div class="typed-parameter">
                ...
                <input type="text" id="node-input-parameter3" placeholder="">
            </div>
            <div class="form-tips paramtip" id="tip-parameter3" hidden></div>
        </div>
    </div>
    <div class="form-row jimp-parameter-group" id="parameter4" style="vertical-align: top;">
        <div style="float:left;">
        ...
        <div class="input-tip-group">
            <div class="typed-parameter">
                ...
                <input type="text" id="node-input-parameter4" placeholder="">
            </div>
            <div class="form-tips paramtip" id="tip-parameter4" hidden></div>
        </div>
    </div>
    <div class="form-row jimp-parameter-group" id="parameter5" style="vertical-align: top;">
        <div style="float:left;">
        ...
        <div class="input-tip-group">
            <div class="typed-parameter">
                ...
                <input type="text" id="node-input-parameter5" placeholder="">
            </div>
            <div class="form-tips paramtip" id="tip-parameter5" hidden></div>
        </div>
    </div>
    <div class="form-row jimp-parameter-group" id="parameter6" style="vertical-align: top;">
        <div style="float:left;">
        ...
        <div class="input-tip-group">
            <div class="typed-parameter">
                ...
                <input type="text" id="node-input-parameter6" placeholder="">
            </div>
            <div class="form-tips paramtip" id="tip-parameter6" hidden></div>
        </div>
    </div>
    <div class="form-row jimp-parameter-group" id="parameter7" style="vertical-align: top;">
        <div style="float:left;">
        ...
        <div class="input-tip-group">
            <div class="typed-parameter">
                ...
                <input type="text" id="node-input-parameter7" placeholder="">
            </div>
            <div class="form-tips paramtip" id="tip-parameter7" hidden></div>
        </div>
    </div>
    <div class="form-row jimp-parameter-group" id="parameter8" style="vertical-align: top;">
        <div style="float:left;">
        ...
        <div class="input-tip-group">
            <div class="typed-parameter">
                ...
                <input type="text" id="node-input-parameter8" placeholder="">
            </div>
            <div class="form-tips paramtip" id="tip-parameter8" hidden></div>
        </div>
    </div>
</script>
