...
$setPubTopic0 = $rec['setPubTopic0'];
		...
		//$query = "update pushover set user_key = '{$user_key}', device = '{$device}', habilitado='{$habilitado}' where id = '{$id}'";
		 $query = "INSERT INTO `autohome`.`rx433mhz` (`id`, `id_widget`, `codigo`, `topico` , `topico_confirma`, `carga`, `descricao`,  `estado`, `habilitado`) VALUES 
		                                      (NULL, '{$id_widget}', '{$codigo}', '{$setSubTopic0}', '{$setPubTopic0}', '0', '{$Descricao}',  '{$estado}', '{$habilitado}');";
...
<label class="w3-label w3-text-blue"><b>Estado</b></label>
 <select name="estado" id="estado" class="w3-input w3-border" >
  <option value="X">Alternar</option>
  <option value="1">Somente Ligar</option>
  <option value="0">Somente Desligar</option>
</select> 
