...
$setPubTopic0 = $rec['setPubTopic0'];
		...
		$query = "update `autohome`.`rx433mhz_portas` set `codigo`= '{$codigo}', `id_widget`= '{$id_widget}', `topico` = '{$setPubTopic0}', `topico_confirma` = '{$setSubTopic0}', `descricao` =  '{$Descricao}',  `carga` =  '{$estado}', `habilitado` =  '{$habilitado}'  where id = '{$id}'";
...
$query = "SELECT id, Descricao, setName0, setSubTopic0, setPubTopic0  FROM `widget` where widget.tipo_geral = 17";
mysqli_set_charset('utf8');
$studentData = mysqli_query($con, $query);

$query = "SELECT * FROM `rx433mhz_portas` where id ={$id}";
mysqli_set_charset('utf8');
...
<label class="w3-label w3-text-blue"><b>Estado</b></label>
 <select name="estado" id="estado" class="w3-input w3-border" >
  <option value="1" <?php    if ($estado_default == "1"){echo " selected='selected'";}  ?> >Aberta</option>
  <option value="0" <?php    if ($estado_default == "0"){echo " selected='selected'";}  ?> >Fechada</option>
</select> 
