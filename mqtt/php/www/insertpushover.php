...
if(isset($_POST['btnSubmit'])) 
	{
		...
		//$query = "update pushover set user_key = '{$user_key}', device = '{$device}', habilitado='{$habilitado}' where id = '{$id}'";
		 $query = "INSERT INTO `autohome`.`pushover` (`id`, `user_key`,  `token_key`,`device`, `email`, `habilitado`) VALUES (NULL, '{$user_key}', '{$token_key}', '{$device}', '{$email}', '{$habilitado}');";
	//	echo $query;
...
<input type="text" class="w3-input w3-border" value="" name="user_key"/> <br/>
<label class="w3-label w3-text-blue"><b>Token Key</b></label>
<input type="text" class="w3-input w3-border" value="" name="token_key"/> <br/>
<label class="w3-label w3-text-blue"><b>Device</b></label>
<input type="text" class="w3-input w3-border" value="" name="device"/> <br/>
