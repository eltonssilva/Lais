...
console.log('Sensor de Ninel de Luz Conectado ao servidor MQTT broker');
client.subscribe('{$topicosub1}');
client.on('message', function(topic, message) {
 // console.log('Temp = ' + message);
//  console.log('Até Aqui ok');
  NurseryLight = message * 1;
  
  FAKE_SENSOR.randomizeLight(); 
  
 sensor
.getService(Service.LightSensor)
.setCharacteristic(Characteristic.CurrentAmbientLightLevel, FAKE_SENSOR.CurrentAmbientLightLevel);
...
// here's a fake Light sensor device that we'll expose to HomeKit
var FAKE_SENSOR = {
  CurrentAmbientLightLevel: NurseryLight,
  getLight: function() { 
  //  console.log(NurseryLight);
    return FAKE_SENSOR.CurrentAmbientLightLevel;
  },
  randomizeLight: function() {
    // randomize Light to a value between 0 and 100
    FAKE_SENSOR.CurrentAmbientLightLevel = NurseryLight;
 //   console.log(NurseryLight);
...
// Generate a consistent UUID for our Light Sensor Accessory that will remain the same
// even when restarting our server. We use the `uuid.generate` helper function to create
// a deterministic UUID based on an arbitrary 'namespace' and the string 'Light-sensor'.
...
// Add the actual LightSensor Service.
...
sensor
  ...
  .on('get', function(callback) {
    ...
    callback(null, FAKE_SENSOR.getLight());
  });

// randomize our Light reading every 3 seconds
";
